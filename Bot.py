...
ADMIN_SECRET = os.getenv("manyteam")

# Контакт администратора для пополнения
ADMIN_USERNAME = "@Pitilookama"
...
class GarantBot:
    ...
    def setup_handlers(self):
        ...
        self.application.add_handler(CommandHandler("deposit", self.deposit_info))
    ...
    async def deposit_info(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            "💳 **Пополнение баланса**\n\n"
            "Для пополнения баланса пиши сюда:\n"
            f"📞 {ADMIN_USERNAME}\n\n"
            "📋 **Инструкция:**\n"
            "1. Напишите администратору\n"
            "2. Укажите ваш ID\n"
            "3. Укажите сумму пополнения\n"
            "4. После пополнения проверьте баланс\n\n"
        ...
        try:
            ...
            if amount > user_balance:
                ...
                await update.message.reply_text(
                    ...
                    f"💸 Требуется: {amount} ₽\n\n"
                    f"Пополните баланс для создания сделки:",
                    reply_markup=reply_markup
                )
                return
                ...
                try:
                    await context.bot.send_message(
                        ...
                        text=f"🎉 Ваш баланс пополнен на {amount} ₽!\n\n"
                             ...
                             f"Для проверки баланса используйте /balance"
                    )
                except:
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "balance":
            ...
            await self.show_deposit_info(query)
    ...
    async def show_deposit_info(self, query):
        user_id = query.from_user.id
        balance = db.get_balance(user_id)
    ...
    async def list_deals_for_query(self, query):
        ...
        await query.edit_message_text(deals_text, reply_markup=reply_markup, parse_mode='Markdown')
    ...
    async def cancel_deal(self, deal_id: str, user_id: int, query):
        ...
        if deal[5] != 'created':  # Можно отменять только созданные
            await query.edit_message_text("❌ Нельз
